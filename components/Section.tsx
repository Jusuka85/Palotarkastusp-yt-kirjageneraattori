...
const Section: React.FC<SectionProps> = ({ title, children, defaultOpen = false, hasDeficiency = false, hasRecommendation = false, isOk = false, onHide }) => {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  const baseButtonClasses = "w-full text-left p-4 flex justify-between items-center transition-colors";
  const defaultBgClasses = "bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700";
  ...
  const baseTitleClasses = "text-xl font-semibold";
  const defaultTitleClasses = ""; // Inherits from body
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md mb-6 overflow-hidden">
      <div className={`${buttonClasses} flex items-center justify-between group cursor-pointer`}>
        ...
        >
          ...
          <svg
            ...
            className={`h-6 w-6 transform transition-transform text-slate-800 dark:text-slate-200 ml-2 ${isOpen ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div className="flex items-center gap-2 pr-2">
            {onHide && (
                 <button 
                    ...
                    className="p-1 rounded-full text-slate-400 dark:text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600 hover:text-slate-700 dark:hover:text-slate-200 transition-opacity"
      ...
      {isOpen && <div className="p-6 border-t border-slate-200 dark:border-slate-700">{children}</div>}
    </div>
  );
};
